// This file are autogenerated on build, everything you write here will be replaced

#![allow(unknown_lints)]
#![allow(clippy::all)]
use crate::generated_schemas::prelude::*;
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PromoCodesCreateCodeParamsGivesCurrenciesParams {
    #[serde(rename = "LIS")]
    pub lis: f64,
    #[serde(rename = "ETH")]
    pub eth: f64,
}
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PromoCodesCreateCodeParamsGivesItemsParams(pub Value);
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PromoCodesCreateCodeParams {
    #[serde(rename = "givesCurrencies")]
    pub gives_currencies: PromoCodesCreateCodeParamsGivesCurrenciesParams,
    #[serde(rename = "givesItems")]
    pub gives_items: PromoCodesCreateCodeParamsGivesItemsParams,
    #[serde(rename = "numberOfAttempts")]
    pub number_of_attempts: f64,
    #[serde(rename = "expiresIn", deserialize_with = "deserialize_to_string")]
    pub expires_in: String,
    #[serde(rename = "code", deserialize_with = "deserialize_to_string")]
    pub code: String,
}
impl Schema for PromoCodesCreateCodeParams {
    fn schema() -> Value {
        serde_json :: from_str ("{\"type\":\"object\",\"properties\":{\"givesCurrencies\":{\"type\":\"object\",\"properties\":{\"LIS\":{\"type\":\"integer\",\"minimum\":-9223372036854775808,\"maximum\":9223372036854775807,\"additionalAttributes\":{\"numberType\":\"Number\"}},\"ETH\":{\"type\":\"integer\",\"minimum\":-9223372036854775808,\"maximum\":9223372036854775807,\"additionalAttributes\":{\"numberType\":\"Number\"}}},\"required\":[\"ETH\",\"LIS\"]},\"givesItems\":{\"type\":\"object\",\"properties\":{}},\"numberOfAttempts\":{\"type\":\"integer\",\"minimum\":-9223372036854775808,\"maximum\":9223372036854775807,\"additionalAttributes\":{\"numberType\":\"Number\"}},\"expiresIn\":{\"type\":\"string\"},\"code\":{\"type\":\"string\"}},\"required\":[\"code\",\"numberOfAttempts\",\"expiresIn\",\"givesCurrencies\",\"givesItems\"]}") . unwrap ()
    }
}
impl Agent for PromoCodesCreateCodeParams {
    fn topic() -> &'static str {
        "promo_codes_createCode"
    }
    fn method() -> &'static str {
        "codes_createCode"
    }
    fn agent() -> &'static str {
        "promo"
    }
    fn access_level() -> AccessLevel {
        AccessLevel::Private
    }
}
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct PromoCodesCreateCodeReturns(pub bool);
impl Schema for PromoCodesCreateCodeReturns {
    fn schema() -> Value {
        serde_json::json!("{\"type\":\"boolean\"}")
    }
}
impl Agent for PromoCodesCreateCodeReturns {
    fn topic() -> &'static str {
        "promo_codes_createCode"
    }
    fn method() -> &'static str {
        "codes_createCode"
    }
    fn agent() -> &'static str {
        "promo"
    }
    fn access_level() -> AccessLevel {
        AccessLevel::Private
    }
}
