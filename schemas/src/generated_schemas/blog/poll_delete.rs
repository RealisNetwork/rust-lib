// This file are autogenerated on build, everything you write here will be replaced

#![allow(unknown_lints)]
#![allow(clippy::all)]
use crate::generated_schemas::prelude::*;
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlogPollDeleteParams {
    #[serde(rename = "id")]
    pub id: f64,
}
impl Schema for BlogPollDeleteParams {
    fn schema() -> Value {
        serde_json :: from_str ("{\"type\":\"object\",\"properties\":{\"id\":{\"type\":\"integer\",\"minimum\":-9223372036854775808,\"maximum\":9223372036854775807,\"additionalAttributes\":{\"numberType\":\"Number\"}}},\"required\":[\"id\"]}") . unwrap ()
    }
}
impl Agent for BlogPollDeleteParams {
    fn topic() -> &'static str {
        "blog_poll_delete"
    }
    fn method() -> &'static str {
        "poll_delete"
    }
    fn agent() -> &'static str {
        "blog"
    }
}
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct BlogPollDeleteReturns(pub bool);
impl Schema for BlogPollDeleteReturns {
    fn schema() -> Value {
        serde_json::json!("{\"type\":\"boolean\"}")
    }
}
impl Agent for BlogPollDeleteReturns {
    fn topic() -> &'static str {
        "blog_poll_delete"
    }
    fn method() -> &'static str {
        "poll_delete"
    }
    fn agent() -> &'static str {
        "blog"
    }
}
