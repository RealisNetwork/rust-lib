...
use error_registry::generated_errors::{GeneratedError, Nats as NatsError};
...
impl Transport for Jet {
    ...
    async fn publish(&self, topic: &str, message: Self::Message, _topic_res: Option<String>) -> Result<(), Self::Error> {
        self.jet_stream
            ...
            .map_err(|_| BaseError::new("Error Send".to_string(),None,None, ErrorType::Generated(GeneratedError::Nats(NatsError::Send))))?;//RealisErrors::Nats(NatsError::Send))?;
        Ok(())
    }
    ...
    async fn subscribe_with_timeout<'a>(
        ...
        let _stream_info = self
            .jet_stream
            .add_stream(topic)
            .map_err(|_| BaseError::new("Error Disconnected".to_string(),None,None, ErrorType::Generated(GeneratedError::Nats(NatsError::Receive))))?;//RealisErrors::Nats(NatsError::Disconnected))?;
    ...
    async fn unsubscribe(&self, subscribe_id: Self::SubscribeId) -> Result<(), Self::Error> {
        subscribe_id.unsubscribe().map_err(|_| BaseError::new("Error Unsubscribe".to_string(),None,None, ErrorType::Generated(GeneratedError::Nats(NatsError::Send))))//RealisErrors::Nats(NatsError::Unsubscribe))
    }

    async fn ok(&self, message_id: Self::MessageId) -> Result<(), Self::Error> {
        message_id.ack().map_err(|_| BaseError::new("Error OK".to_string(),None,None, ErrorType::Generated(GeneratedError::Nats(NatsError::Send))))//RealisErrors::Nats(NatsError::Ok))
    }
