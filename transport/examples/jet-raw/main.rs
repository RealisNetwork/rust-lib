	...
	let publish_thread_join_handle = thread::spawn(move || {
		...
		match jet_nats::connect(NATS_URL) {
			Ok(nats) => {
				...
				println!("[{:^15}] - Publishing by topic: `{}`, message: `{:?}`", THREAD_PREFIX, topic, message);
				match jetstream.publish(topic, message) {
					Ok(publish_ack) => {
						println!("[{:^15}] - Successfully published to nats! Some publish info: {:#?}", THREAD_PREFIX, publish_ack);
					}
					Err(error) => {
		...
		match jet_nats::connect(NATS_URL) {
			Ok(nats) => {
				...
				match jetstream.add_stream(TOPIC) {
					Ok(stream_info) => {
						println!("[{:^15}] - Successfully created stream to nats! Some stream info: {:#?}", THREAD_PREFIX, stream_info);
