...
mod rebuild {
    ...
    pub fn rebuild() {
        ...
        let map: HashMap<String, HashMap<String, Value>> =
            serde_json::from_str(json.as_str()).unwrap();

        let mut res2: HashMap<String, Vec<ErrorParams>> = HashMap::new();

        for (header, params) in map {
            let head: Vec<String> = header.split(".").map(|s| s.to_string()).collect(); // index 0 - prefix, index 1 - postfix
        ...
        for enum_name in res2.keys() {
            ...
            let postfix_copy_1 = postfix.clone();
            let postfix_copy_2 = postfix.clone();
            let full_names = res2
                .get(enum_name)
                .unwrap()
                .iter()
            ...
            code.extend(quote!(
                ...
                impl ToString for #enum_name_indent {
                    fn to_string(&self) -> String {
                        match self {
                            #(
                                Self::#postfix_copy_1=>#full_names.to_string(),
                            )*
                        }
                    }
                }
                ...
                impl From<#enum_name_indent> for u32 {
                    fn from(error_type: #enum_name_indent) -> u32 {
                        match error_type {
                            #(
                                #enum_name_indent::#postfix_copy_2=>#error_code,
                            )*
                        }
                    }
